...
public:
	...
	virtual bool OnEvent(const irr::SEvent & event)
	{
		...
		if (event.EventType == irr::EET_KEY_INPUT_EVENT)
		{
			...
			if (KeyIsDown[irr::KEY_KEY_W]) { paddle1Up = true; } // set true when key is pressed on keyboard
			else { paddle1Up = false; } // otherwise set to false
			if (KeyIsDown[irr::KEY_KEY_S]) { paddle1Down = true; }
			else { paddle1Down = false; }

			// Paddle2 control values.
			if (KeyIsDown[irr::KEY_KEY_O]) { paddle2Up = true; }
			else { paddle2Up = false; }
			if (KeyIsDown[irr::KEY_KEY_L]) { paddle2Down = true; }
